...
public class PathChunkFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public static Optional<EvaluatedOrigin> evaluateOriginCachedOnly(
        ...
        if(fastCached != null) {
            // Path is already done. Fast-path: skip biome filter (cached implies it passed)
            // but we still need to know which network we're rasterising for. Re-run
            // selectNetworkAt - it's only one biome lookup against the existing cache.
        ...
        // Cache miss. Apply the biome filter so we don't enqueue garbage origins, and
        // if it passes, hand the job to the deferred worker.
        ...
        if(networkId == null) {
            // network instance not in the registry by key - shouldn't happen, but bail safely
            return Optional.empty();
        }
