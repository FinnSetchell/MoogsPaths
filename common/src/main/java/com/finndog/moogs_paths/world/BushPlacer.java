...
public final class BushPlacer {
    ...
    public static void placeInChunk(WorldGenLevel level, List<BlockPos> waypoints, List<Identifier> bushSetIds, HolderSet<Biome> biomes, int chunkX, int chunkZ, RandomSource random, int flags) {
        ...
        WaypointScatterer.scatter(waypoints, set.density(), maxReach, chunkX, chunkZ, random,
            (from, d, parX, parZ, perpX, perpZ, segRandom) -> {
                ...
                if(cx + size >= chunkMinX && cx - size <= chunkMaxX && cz + size >= chunkMinZ && cz - size <= chunkMaxZ) {
                    ...
                    if(Constants.ENABLE_DEBUG_TIMER) PathDataManager.recordBiomeCall(com.finndog.moogs_paths.data.BiomeCallSite.BUSH_PLACE_CHECK);
